...
def get_user_ip():
    ...
    ip = "Unknown"
    # พยายามดึงจาก X-Forwarded-For (ที่ Streamlit Cloud ใช้)
    if "X-Forwarded-For" in headers:
        # แยกเอาตัวแรกสุด และ .strip() เพื่อลบช่องว่างทิ้ง
...
def get_location_info(ip):
    # ถ้าเทสในเครื่องตัวเอง หรือหา IP ไม่เจอ
    if ip == "Unknown" or ip.startswith("127.") or ip.startswith("172."):
        return "Local Machine / Private IP"
        
    try:
        # เรียก API เช็คพิกัด (ใส่ timeout กันค้าง)
        ...
        print(f"❌ API Error: {e}") # ดูในหน้า Logs ดำๆ ว่ามันฟ้องอะไร
        pass
    return "ไม่ทราบพิกัด (ข้อมูลไม่ระบุ)"
