...
def get_user_ip():
    ...
    # พยายามดึง IP จริงจาก Header หลายๆ ช่องทาง
    ip = "127.0.0.1"
...
def get_location_info(ip):
    # ถ้าเป็น IP เครื่องตัวเอง ไม่ต้องไปหาพิกัด
    if ip == "127.0.0.1" or ip == "Unknown":
        return "Local Machine (คุณกำลังเทสเอง)"
        
    try:
        # ใช้ API ดึงข้อมูล จังหวัด และ ประเทศ
        ...
        print(f"Location Error: {e}") # พ่น Error บอกในหน้า Logs
        pass
    return "ไม่ทราบพิกัด (อาจใช้ VPN หรือ Proxy)"
