...
def get_user_ip():
    try:
        # วิธีดักจับ IP จริงจากภายนอก (แม่นยำที่สุด)
        ip = requests.get('https://api.ipify.org', timeout=5).text
        return ip.strip()
    except:
        # ถ้าดึงจากข้างนอกไม่ได้ ให้ลองดึงจาก Header ของ Streamlit
...
def get_location_info(ip):
    if ip == "Unknown" or ip.startswith("10.") or ip.startswith("192.168."):
        return "Local Network / VPN"
        
    try:
        # ดึงพิกัดจาก ip-api (ตัวนี้เสถียรสุดสำหรับไทย)
        url = f"http://ip-api.com{ip}?fields=status,message,country,regionName,city"
        response = requests.get(url, timeout=10).json()
        
        if response.get('status') == 'success':
            city = response.get('city', 'ไม่ระบุเมือง')
            region = response.get('regionName', 'ไม่ระบุจังหวัด')
            country = response.get('country', 'ไทย')
            return f"{city}, {region}, {country}"
    except Exception as e:
        print(f"Location Error: {e}")
        
    return "ไม่ทราบพิกัด (ระบบพรางตัว)"
