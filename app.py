...
# --- ตั้งค่าหน้าเว็บ ---
def get_user_ip():
    # วิธีที่ 1: ดึง IP จริงผ่าน API ภายนอก (แม่นที่สุดสำหรับมือถือ)
    try:
    ...
    # วิธีที่ 2: ดึงจาก Streamlit Header (กรณีวิธีแรกช้า)
    headers = st.context.headers
    forwarded = headers.get("X-Forwarded-For", "")
    if forwarded:
        # เอา IP ตัวแรกสุดที่เจอ
        return forwarded.split(",")[0].strip()
    return "Unknown"

def get_location_info(ip):
    # ไม่มีการดักว่าเทสเองแล้ว เจอเลขอะไรส่งไปเช็คพิกัดทันที!
    if not ip or ip == "Unknown":
        return "พิกัดลับ (ตรวจจับไม่ได้)"
        
    try:
        # ใช้ API ip-api (รองรับไทยได้ดีที่สุด)
        ...
        pass
    return f"ตรวจพบจาก IP: {ip} (พิกัดถูกพราง)"
