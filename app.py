...
def get_user_ip():
    ...
    # ดึงค่า X-Forwarded-For ซึ่งเป็นมาตรฐานของ Streamlit Cloud
    forwarded = headers.get("X-Forwarded-For", "")
    if forwarded:
        # แยกเอาตัวแรกสุด ลบช่องว่าง และตัดส่วนพอร์ต (ถ้ามี) ออกให้เหลือแค่เลข IP
        ip = forwarded.split(",")[0].strip()
        return ip
    return "Unknown"

def get_location_info(ip):
    # ถ้าเทสในเครื่องตัวเอง หรือหา IP ไม่เจอ ให้ข้ามไปเลย
    if ip == "Unknown" or ip.startswith("127.") or ip.startswith("172.") or ip.startswith("192."):
        return "Local Machine (คุณกำลังเทสเอง)"
        
    try:
        # ใช้ API สำรอง (ipapi.co) ซึ่งเสถียรกว่าในบางพื้นที่
        url = f"https://ipapi.co{ip}/json/"
        # ใส่หัวข้อ User-Agent เพื่อไม่ให้ API บล็อกเรา
        ...
        if "city" in response:
            ...
            country = response.get("country_name", "ไม่ระบุประเทศ")
            return f"{city}, {region}, {country}"
    except Exception as e:
        print(f"❌ Error Fetching Location: {e}")
        
    return "ไม่ทราบพิกัด (ระบบป้องกัน IP)"
